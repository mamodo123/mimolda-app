...
class ProbationOrdersScreen extends StatefulWidget {
  const ProbationOrdersScreen({super.key});

  @override
  State<ProbationOrdersScreen> createState() => _ProbationOrdersScreenState();
}

class _ProbationOrdersScreenState extends State<ProbationOrdersScreen>
    with AutomaticKeepAliveClientMixin<ProbationOrdersScreen> {
...
class _PurchaseOrdersScreenState extends State<PurchaseOrdersScreen>
    with AutomaticKeepAliveClientMixin<PurchaseOrdersScreen> {
  static const orderByOptions = ['Atualização', 'Data do pedido'];
  static final filterByOptions = [
    'Todos',
    ...purchaseStatusTranslator.keys,
    'Expirado',
    // 'Entrega próxima'
  ];
  var orderBy = orderByOptions.first;
  ...
  Widget build(BuildContext context) {
    ...
    final fullStoreNotifier = context.watch<FullStoreNotifier>();
    final ordersWithoutFilterAndSort =
        List<MimoldaOrder>.from(fullStoreNotifier.user?.purchases ?? []);
    final orders = filterAndSortOrder(ordersWithoutFilterAndSort);
                  ...
                  return GestureDetector(
                    ...
                    child: Padding(
                      ...
                      child: Card(
                        color: order.isExpired
                            ? null
                            : purchaseStatusColor[order.status],
                        child: Padding(
                          padding: const EdgeInsets.all(16).copyWith(bottom: 8),
                          child: Stack(
                            children: [
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    if (order.deliveryDate != null)
                                      Text(
                                          'Data de ${order.address?.id.isEmpty ?? false ? 'retirada' : 'entrega'}: ${DateFormat('dd/MM/yyyy').format(order.deliveryDate!.toUtc())}, ${order.period}',
                                          textAlign: TextAlign.center),
